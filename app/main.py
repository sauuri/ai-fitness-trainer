...
<div class="card">
  ...
  실명 등 식별 정보는 수집하지 않으며, 운동 기록은 기기와 익명 식별자 기반 클라우드 동기화에 저장됩니다.<br>
...
<h2>1. 수집하는 정보</h2>
<p>앱은 이름·이메일·전화번호 등 실명 식별 정보를 수집하지 않습니다. 프로필(나이·성별·체중·신장·운동 목표 등) 및 운동 기록만 수집합니다.</p>

<h2>2. 저장 및 동기화 방식</h2>
<p>위 정보는 기본적으로 사용자 기기의 <strong>로컬 저장소(localStorage)</strong>에 저장됩니다. 또한 기기 간 데이터 동기화(백업·복원)를 위해 임의로 생성된 <strong>익명 기기 식별자(device ID)</strong>와 함께 클라우드 서버(Supabase)에도 동일한 데이터가 저장됩니다. 이 식별자는 실명·이메일 등과 연결되지 않는 무작위 문자열입니다.</p>
...
<h2>4. 처리 위탁</h2>
<p>앱은 다음 업체에 데이터 처리를 위탁하고 있습니다: <strong>Supabase Inc.</strong>(클라우드 동기화·저장), <strong>OpenAI</strong>(AI 루틴/분석 생성). 각 업체는 위탁받은 목적 외로 데이터를 사용하지 않습니다.</p>

<h2>5. 제3자 제공</h2>
<p>앱은 사용자의 데이터를 위 처리 위탁업체 외 제3자에게 판매하거나 제공하지 않습니다.</p>

<h2>6. 데이터 삭제</h2>
<p>앱 내 <strong>프로필 탭 → 전체 초기화</strong>를 실행하면 기기 로컬 데이터와 클라우드(Supabase)에 동기화된 데이터가 함께 삭제됩니다. 단순히 앱을 삭제하는 경우 기기 로컬 데이터는 삭제되지만 클라우드 동기화 데이터는 남아있을 수 있으므로, 완전한 삭제를 원하시면 앱 삭제 전 전체 초기화를 먼저 실행하거나 아래 이메일로 삭제를 요청해 주세요.</p>
...
<p class="badge">최종 업데이트: 2026년 6월 19일</p>
</body>
</html>"""
    from fastapi.responses import HTMLResponse
    return HTMLResponse(html)
...
한국 음식 칼로리 기준 (수량 명시 없으면 아래 단위 그대로 적용):

[치킨 — 1마리 = 통닭 전체]
- 교촌 허니콤보 1마리 ≈ 2500kcal
- 교촌 허니오리지날/레드콤보 1마리 ≈ 2200kcal
- BHC 뿌링클 1마리 ≈ 2300kcal
- BBQ 황금올리브 1마리 ≈ 2400kcal
- 일반 후라이드/양념 1마리 ≈ 1800~2000kcal
- 반마리 명시 없으면 무조건 통마리 기준

[마라탕/마라샹궈 — 무게 기준]
- 마라탕 100g ≈ 150~200kcal (기름량 따라 변동 큼)
- 마라탕 보통 그릇(400~600g) ≈ 700~1100kcal
- 마라샹궈 100g ≈ 220~280kcal (볶음이라 기름 더 많음)
- 마라샹궈 보통 주문(500~700g) ≈ 1100~1700kcal
- "1인분" 명시 시 500g 기준으로 계산

[피자 — 판/조각 기준]
- 대형 피자(L/XL) 1판(8조각) ≈ 2000~2400kcal
- 1조각 ≈ 250~300kcal
- "1판" = 8조각 전체

[족발/보쌈]
- 족발 소 ≈ 1200kcal / 중 ≈ 1800kcal / 대 ≈ 2400kcal
- 보쌈 소 ≈ 1000kcal / 중 ≈ 1500kcal / 대 ≈ 2000kcal

[라면/컵라면]
- 봉지라면 1개(신라면·짜파게티 등) ≈ 500~550kcal
- 컵라면 1개 ≈ 300~380kcal
- 라면 + 밥 추가 시 +300kcal

[삼겹살/고기]
- 삼겹살 1인분(200g) ≈ 700kcal
- 목살 1인분(200g) ≈ 600kcal
- 소고기 등심 1인분(150g) ≈ 400kcal

[분식]
- 떡볶이 1인분 ≈ 500~600kcal
- 순대 1인분 ≈ 300~400kcal
- 엽기떡볶이 1인분(소) ≈ 800kcal / 한통(2인분) ≈ 1600~1800kcal

[국밥/찌개류]
- 순대국밥/설렁탕/곰탕 1인분 ≈ 600~700kcal
- 부대찌개 1인분 ≈ 700~900kcal
- 김치찌개 1인분(밥 제외) ≈ 250kcal / 밥 포함 ≈ 550kcal

[햄버거/패스트푸드]
- 맥도날드 빅맥 단품 ≈ 530kcal / 세트 ≈ 1000kcal
- 버거킹 와퍼 단품 ≈ 630kcal / 세트 ≈ 1100kcal
- "세트" 명시 시 음료+감자튀김 포함 계산
